...
class LKGModelWrapper:
    def __init__(self, config):
        ...
        logger.info(f"Loading Base Model: {config['model']['name']} on {self.device} (FP16)")
        ...
        sae_path_base = config['sae']['release']
        sae_id = config['sae']['id']
        full_path = os.path.join(sae_path_base, sae_id)
        ...
        state_dict = torch.load(os.path.join(full_path, "sae_weights.pt"), map_location=self.device)
        ...
        self.activation_threshold = self.config['pipeline'].get('activation_threshold', 1.0)
    ...
    def get_active_features(self, tokens: torch.Tensor) -> List[Set[int]]:
        ...
        def sae_hook(resid_pre, hook):
            ...
            batch_sets = [[set() for _ in range(tokens.shape[1])] for _ in range(tokens.shape[0])]
        ...
        with torch.no_grad():
            self.model.run_with_hooks(
                ...
                stop_at_layer=self.config['model']['target_layer'] + 1
            )
